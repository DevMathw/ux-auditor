...
${audit.strengths ? `<h2>${esc(t.strengths)}</h2><div class="callout">${esc(audit.strengths)}</div>` : ""}
  ...
  return (
    <div className="export-group" role="group" aria-label={t.exportGroupLabel}>
      ...
      <button className="rerun-btn" onClick={() => download(toJson(audit, url), exportFilename(url, "json"), "application/json")}>
        ↓ {t.exportJson}
      </button>
      <button className="rerun-btn" onClick={() => download(toMarkdown(audit, url), exportFilename(url, "md"), "text/markdown")}>
        ↓ {t.exportMarkdown}
      </button>
    </div>
  );
